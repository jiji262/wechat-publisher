...
**统一使用手绘蓝色信息图风格**,不使用混合策略,风格一致就是这个 skill 的视觉指纹。
...
**风格要点**:
- 画风:手绘线条风,干净简约,有科技插画感
- 背景:纯白
- 主色:蓝色 `#4a6cf7`(与排版主题色一致)
- 辅助色:橙色 `#ff8c42` 等对比色,整体不超过 2-3 种
- 标注:中文,字体清晰
- **禁忌**:写实照片、3D 渲染、深色背景、花哨装饰

#### 不同类型的 prompt 侧重

- **架构图**:层次(Top/Middle/Bottom)、箭头流向、组件名标注
- **对比图**:左右分栏,各自标注,中间 vs / 箭头
- **流程图**:步骤编号、箭头串联
- **趋势图**:时间线、里程碑节点、方向箭头
...
| 脚本 | 用途 |
|---|---|
| `publish.py` | 完整发布流程(一键,含 AI 味 gate) |
| `wechat_api.py` | **facade(向后兼容)** —— 重导出下述模块 + 提供 CLI |
| `config.py` | (内部)`accounts.yaml` 加载 + `ConfigError` + `set_account` / `get_config` |
| `wechat_token.py` | (内部)`get_access_token`,本地文件缓存 |
| `api.py` | (内部)图片上传 / 草稿创建 / 发布 |
| `html_converter.py` | Markdown → 微信 HTML(多主题 + 行内标色) |
| `image_handler.py` | 图片下载 / 上传 / 替换 |
| `ai_score.py` | **反 AI 检测自检**(publish.py 自动调用,也可单独跑) |
