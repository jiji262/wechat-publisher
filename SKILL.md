...
### ⭐🔥 手绘水彩信息图系列(v3 重写 · **贴图模式默认**)

**对标参考图**:[GPT-5.5 来了](https://mp.weixin.qq.com/s/nSlGjh9hjM63jxz_jgUCwA) —— 这是一种**中文手绘水彩插画信息图**风格(NOT Bloomberg flat vector,NOT 3D),像一页日本/台湾科普绘本或杂志插页。

**共享视觉语言**(4 种配色都是同一套,只换色板):
- 9:16 竖版 1080x1920
- **手绘水彩 + 墨线**,软纸张纹理、淡笔触
- **卡通机器人 + 男孩**两个角色贯穿所有卡(姿势随内容变化)
- **8 个必含版面区块**:
  1. 顶部深色小标签(话题 + 卡片序号)
  2. 粗体手绘大标题(中文,带厚重描边)
  3. 副标题胶带横条
  4. 中部水彩插画场景(角色 + 道具 + 弯曲编号路径 + 便利贴)
  5. 仿 macOS terminal 黑条(短英文命令)
  6. 2×2 要点小卡网格(4 个圆数字子点 + 小图标)
  7. 底部 3-4 个彩色胶囊标签
  8. 页脚淡水印

**4 种配色按话题气质选**:
- ⭐ **`infographic-warm`**(**默认,参考图同款**)—— 暖米黄 + 红橙手绘 + 琥珀辅色。通用、AI 产品、工具、人文
- **`infographic-blue`** —— 冷蓝 + 橙强调。SDK/协议/基建/商务拆解
- **`infographic-dark`** —— 深靛蓝夜 + 霓虹青黄。前沿模型/安全/赛博
- **`infographic-mint`** —— 薄荷 + 珊瑚。生产力/方法论/学习笔记
...
**⚠️ 重要提醒**:
- infographic 系列需要 **brief.md 的要点里有具体信息**(数字、名词、对比、步骤名),AI 才能把子点渲染成 2×2 网格里的真内容。要点越具体,生成的信息图越真实。
...
**不需要在 brief.md 里写 `image_style`**,贴图模式的默认兜底是 `infographic-warm` —— 对标参考文章 [GPT-5.5 来了](https://mp.weixin.qq.com/s/nSlGjh9hjM63jxz_jgUCwA) 的视觉:

- **手绘水彩 + 墨线**,不是 flat vector
- **卡通机器人 + 男孩**两角色贯穿所有卡
- **8 个必含区块**:顶部标签 / 粗体手绘大标题 / 副标题胶带 / 水彩插画场景 / 仿 terminal 黑条 / 2×2 要点小卡 / 彩色胶囊标签带 / 页脚
- 9:16 竖版,信息密度极高

想换配色就在 brief.md 写 `image_style: infographic-blue` / `infographic-dark` / `infographic-mint`(见 [`assets/image-styles/README.md`](assets/image-styles/README.md))。

**账号级别**:`accounts.yaml` 里每个账号可以配 `newspic_image_style`,与文章模式的 `image_style` 分开(因为文章线条手绘和贴图水彩信息图是两种完全不同的视觉语言)。当前默认:
